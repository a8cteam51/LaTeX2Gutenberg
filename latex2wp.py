...
# Unicode equivalents for <mi mathvariant="double-struck"> produced by latex2mathml.
# Using literal Unicode characters is more reliable than the mathvariant attribute.
_DOUBLE_STRUCK = {
...
def _fix_mathml(inner) :
    """Post-process MathML generated by latex2mathml.

    1. Replace <mi mathvariant="double-struck">X</mi> with the literal
       Unicode double-struck character so all browsers render it correctly.
    2. Strip leading/trailing &nbsp; from <mtext> elements — latex2mathml
       converts spaces inside \\hbox{...} to &nbsp;, producing 1em gaps.
    """
    inner = re.sub(
        r'<mi mathvariant="double-struck">([A-Za-z])</mi>',
        lambda m: '<mi>' + _DOUBLE_STRUCK.get(m.group(1), m.group(1)) + '</mi>',
        inner)
    # Normalize all HTML/XML forms of U+00A0 (non-breaking space) to the
    # actual character, then strip from edges and replace internals with space.
    _nbsp = re.compile(r'&nbsp;|&#160;|&#x0*[Aa]0;', re.IGNORECASE)
    def _strip_nbsp(m) :
        content = _nbsp.sub('\u00a0', m.group(2))
        content = content.strip('\u00a0')
        content = content.replace('\u00a0', ' ')
        return m.group(1) + content + m.group(3)
    inner = re.sub(r'(<mtext[^>]*>)(.*?)(</mtext>)', _strip_nbsp, inner, flags=re.DOTALL)
    return inner
